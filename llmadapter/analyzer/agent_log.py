...
class AnalyzerAgent:
    ...
    def process(self, input_dict_1, input_dict_2, sheetnames):
        # Convert dicts to strings for LLM prompt formatting
        ...
        # Reverse lookup: (sheet, address) -> previous (old) value
        address_to_old_value = {}
        for sheet, val_map in input_dict_1.items():
            if not isinstance(val_map, dict):
                raise ValueError(f"Expected dict for sheet '{sheet}', but got: {type(val_map)} — {val_map}")
        ...
        for sheet, fields in input_dict_2.items():
            ...
            for field, new_value in fields.items():
                ...
                # Get suggested cell addresses from LLM output
                addresses = []
